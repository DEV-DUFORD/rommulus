...
#   - drives the CANDIDATE Beetle PCE Fast core against a fully original,
#     deterministic 8 KiB HuCard generated by pce_rom.py. The HuC6280 program
#     maps the core's 2 KiB BRAM, preserves its required HUBM header, and
...
# Candidate Beetle PCE Fast core (NOT advertised in any manifest).
PCE_CORE_ID = "beetle_pce_fast"
PCE_REVISION = "b211204c7026dff6e86e79b00185512e2421fff8"
PCE_SRAM_SIZE = pce_rom.SRAM_SIZE
PCE_ROM_NAME = "rommulus-e2e-beetle-pce-fast.pce"
PCE_RUN1_FRAMES = 240
PCE_RUN2_FRAMES = 180
PCE_RUN3_FRAMES = 60
...
class Runner:
    ...
    def pce_core_filename(self):
        if IS_WINDOWS:
            return "beetle_pce_fast_core.dll"
        if sys.platform == "darwin":
    ...
    def generate_pce_rom(self):
        rom_path = os.path.join(self.cache_root, PCE_ROM_NAME)
        with open(rom_path, "wb") as f:
    ...
    def assert_pce_result(self, name, result, session_id, limit, run_frames):
        problems = validate_result_schema(result)
        self.check(name, not problems, "result schema: %s" % "; ".join(problems))
        if problems:
            return False
        ...
        if not (limit <= frames <= limit + 2):
            return False
        want_image = pce_rom.expected_sram_image(run_frames)
        want_hash = hashlib.sha256(want_image).hexdigest()
        self.check(name, result["saveSize"] == PCE_SRAM_SIZE,
                   "saveSize %r != %d" % (result["saveSize"], PCE_SRAM_SIZE))
        self.check(name, result["saveHash"] == want_hash,
        ...
        if self.check(name, os.path.isfile(candidate),
            ...
            with open(candidate, "rb") as f:
                blob = f.read()
            self.check(name, blob == want_image,
                       "candidate file does not match deterministic 2 KiB BRAM")
        return True
    ...
    # -- Beetle PCE Fast candidate scenarios (qualification gate) -----------

    def _require_pce_candidate(self, name):
        if self.pce_candidate_core is None:
            self.fail(name, "candidate Beetle PCE Fast core not staged")
            return False
        return True

    def scenario_pce_valid_launch_completed(self):
        name = "pce-valid-launch-completed"
        self.scenarios.append({"name": name, "passed": True})
        if not self._require_pce_candidate(name):
            return
        session = "e2e-pce-run1"
        rc, result = self.launch(
            name, session, core_id=PCE_CORE_ID,
            core_build_revision=PCE_REVISION, core_path=self.pce_core_path(),
            content_path=os.path.join(self.cache_root, PCE_ROM_NAME),
            player_max_frames=PCE_RUN1_FRAMES)
        self.check(name, rc == 0, "exit code %r (want 0); see log" % rc)
        if not self.check(name, result is not None, "no result JSON written"):
            return
        if self.assert_pce_result(
                name, result, session, PCE_RUN1_FRAMES, [result["frames"]]):
            self.pce_chain = [result["frames"]]

    def scenario_pce_relaunch_persistence(self):
        name = "pce-relaunch-persistence"
        self.scenarios.append({"name": name, "passed": True})
        if not self._require_pce_candidate(name):
            return
        prev = os.path.join(self.state_root, "e2e-pce-run1.candidate.srm")
        session = "e2e-pce-run2"
        ...
        rc, result = self.launch(
            ...
            core_build_revision=PCE_REVISION, core_path=self.pce_core_path(),
            content_path=os.path.join(self.cache_root, PCE_ROM_NAME),
            player_max_frames=PCE_RUN2_FRAMES)
        self.check(name, rc == 0, "exit code %r (want 0); see log" % rc)
        if not self.check(name, result is not None, "no result JSON written"):
            return
        chain = list(getattr(self, "pce_chain", [])) + [result["frames"]]
        if self.assert_pce_result(name, result, session, PCE_RUN2_FRAMES, chain):
            self.pce_chain = chain

    def scenario_pce_repeated_load(self):
        name = "pce-repeated-load"
        self.scenarios.append({"name": name, "passed": True})
        if not self._require_pce_candidate(name):
            return
        prev = os.path.join(self.state_root, "e2e-pce-run2.candidate.srm")
        session = "e2e-pce-run3"
        if not self.check(name, os.path.isfile(prev), "previous candidate missing"):
            return
        save_path = os.path.join(self.data_root, session, "save.srm")
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        ...
        rc, result = self.launch(
            ...
            core_build_revision=PCE_REVISION, core_path=self.pce_core_path(),
            content_path=os.path.join(self.cache_root, PCE_ROM_NAME),
            player_max_frames=PCE_RUN3_FRAMES)
        self.check(name, rc == 0, "exit code %r (want 0); see log" % rc)
        if not self.check(name, result is not None, "no result JSON written"):
            return
        chain = list(getattr(self, "pce_chain", [])) + [result["frames"]]
        if self.assert_pce_result(name, result, session, PCE_RUN3_FRAMES, chain):
            self.pce_chain = chain

    def scenario_pce_force_kill_lock_recovery(self):
        name = "pce-force-kill-lock-recovery"
        self.scenarios.append({"name": name, "passed": True})
        if not self._require_pce_candidate(name):
            return
        ...
        victim = PlayerProcess(
            self.player_exe, request_path,
            self.env_for(player_max_frames=PCE_KILL_VICTIM_FRAMES),
            os.path.join(self.logs_dir, name + "-victim.log"))
        victim.start()
        pid = victim.pid()
        self.spawned_pids.append(pid)
        ...
        rc, result = self.launch(
            ...
            core_build_revision=PCE_REVISION, core_path=core_path,
            content_path=content_path, player_max_frames=PCE_RUN3_FRAMES)
        self.check(name, rc == 0, "relaunch exit code %r (want 0)" % rc)
        if not self.check(name, result is not None, "relaunch wrote no result JSON"):
            return
                   ...
                   self.scenario_force_kill_lock_release_relaunch,
                    ...
                    self.scenario_gambatte_force_kill_lock_recovery,
                     ...
                     self.scenario_fceumm_force_kill_lock_recovery,
                      ...
                      self.scenario_pce_valid_launch_completed,
        ...
        if not wswan_candidate:
            for cand in (os.path.join(stage_dir, "cores-candidate", "libmednafen_wswan_core.so"),
                ...
                if os.path.isfile(cand):
                    ...
                    break
        pce_candidate = explicit_pce_candidate
        if not pce_candidate:
            for cand in (
                    os.path.join(stage_dir, "cores-candidate",
                                 "libbeetle_pce_fast_core.so"),
                    os.path.join(stage_dir, "cores-candidate",
                                 "beetle_pce_fast_core.dll"),
                    os.path.join(stage_dir, "libbeetle_pce_fast_core.so"),
                    os.path.join(stage_dir, "libbeetle_pce_fast_core.dylib")):
                if os.path.isfile(cand):
                    pce_candidate = cand
                    break
...
def main(argv=None):
    ...
    if wswan_candidate is None:
        ...
        print("FAIL: candidate mednafen_wswan core not found in %r (expected "
              ...
              % args.stage, file=sys.stderr)
        return 2
    if pce_candidate is None:
        print("FAIL: candidate Beetle PCE Fast core not found in %r (expected "
              "cores-candidate/beetle_pce_fast_core.dll on Windows or "
