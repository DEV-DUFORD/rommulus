// core_library.h — dynamic-library loader for one Libretro core shared library.
//
...
public:
    ...
    // Loads path through the platform DynamicLibrary backend (dynamic
    // loading on Android) and resolves every required retro_* symbol.
    // Returns false
    // (and leaves the library unloaded) on any failure.
...
private:
    // Opaque pointer to the romm::dynamiclib::DynamicLibrary owning the loaded core.
