...
public:
    ...
    // Test/diagnostic accessor: the next frame deadline. Exposed so tests can
    // verify that drift recovery snapped the schedule to "now" rather than
    // leaving it stale. Does not affect runtime behavior.
    std::chrono::steady_clock::time_point nextFrameTime() const { return nextFrameTime_; }
