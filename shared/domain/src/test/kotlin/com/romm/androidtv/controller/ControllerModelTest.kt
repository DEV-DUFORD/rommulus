...
class ControllerModelTest {
    ...
    inner class ControllerMappingTests {
        ...
        fun `default has all buttons`() {
            ...
            assertThat(mapping.buttons).containsKey(NeutralKey.BUTTON_A)
            assertThat(mapping.buttons).containsKey(NeutralKey.BUTTON_B)
        }
        ...
        fun `default has all axes`() {
            ...
            assertThat(mapping.axes).containsKey(NeutralAxis.X)
        }
        ...
        fun `swap preserves axes`() {
            val mapping = ControllerMapping(
                buttons = mapOf(
                    NeutralKey.BUTTON_A to LogicalControl.BUTTON_B,
                    NeutralKey.BUTTON_B to LogicalControl.BUTTON_A
                ),
                axes = mapOf(
                    NeutralAxis.X to LogicalControl.AXIS_RX,
                    NeutralAxis.Y to LogicalControl.AXIS_RY
                )
            )
            val swapped = ControllerMapping.swapAB(mapping)
    ...
    inner class GamepadSnapshotTests {
        ...
        fun `physical axes apply deadzone`() {
            val mapping = ControllerMapping()
            val axes = mapOf(
                NeutralAxis.X to 0.1f,
                NeutralAxis.Y to 0.5f
            )
        ...
        fun `inverted axis`() {
            ...
            val axes = mapOf(NeutralAxis.X to 0.5f)
            val snap = GamepadSnapshot.fromPhysicalInput(emptySet(), axes, mapping)
        ...
        fun `trigger axis mapping`() {
            val mapping = ControllerMapping()
            val axes = mapOf(
                NeutralAxis.GAS to 0.8f,
                NeutralAxis.BRAKE to 0.3f
            )
            val snap = GamepadSnapshot.fromPhysicalInput(emptySet(), axes, mapping)
        ...
        fun `hat to dpad buttons`() {
            val mapping = ControllerMapping()
            val pressed = setOf(NeutralKey.DPAD_UP, NeutralKey.DPAD_RIGHT)
            val snap = GamepadSnapshot.fromPhysicalInput(pressed, emptyMap(), mapping)
    ...
    inner class LogicalControlTests {
        ...
        @Test
        @DisplayName("NEUTRAL_KEY_TO_CONTROL maps standard keys")
        fun `keycode mapping`() {
            assertThat(NEUTRAL_KEY_TO_CONTROL[NeutralKey.BUTTON_A])
                .isEqualTo(LogicalControl.BUTTON_A)
            assertThat(NEUTRAL_KEY_TO_CONTROL[NeutralKey.DPAD_UP])
                .isEqualTo(LogicalControl.DPAD_UP)
        }

        @Test
        @DisplayName("NEUTRAL_AXIS_TO_CONTROL maps standard axes")
        fun `axis mapping`() {
            assertThat(NEUTRAL_AXIS_TO_CONTROL[NeutralAxis.X])
                .isEqualTo(LogicalControl.AXIS_LX)
            assertThat(NEUTRAL_AXIS_TO_CONTROL[NeutralAxis.GAS])
    ...
    inner class AngleBasedDpadTests {
        ...
        private fun snap(axisX: Float = 0f, axisY: Float = 0f) =
            GamepadSnapshot.fromPhysicalInput(
                emptySet(),
                mapOf(NeutralAxis.X to axisX, NeutralAxis.Y to axisY),
                dpadMapping
            )
        ...
        fun `non standard dpad target falls back`() {
            val weirdMapping = ControllerMapping(
                axisDirections = mapOf(
                    ...
                    AxisDirection(NeutralAxis.X, -1) to LogicalControl.DPAD_LEFT,
                    AxisDirection(NeutralAxis.Y,  1) to LogicalControl.DPAD_DOWN,
                    AxisDirection(NeutralAxis.Y, -1) to LogicalControl.BUTTON_A // non-standard!
                )
            )
        ...
        fun `non dpad axis direction works`() {
            val mapping = ControllerMapping(
                axisDirections = mapOf(
                    ...
                    AxisDirection(NeutralAxis.Y,  1) to LogicalControl.DPAD_DOWN,
                    AxisDirection(NeutralAxis.Y, -1) to LogicalControl.DPAD_UP,
                    AxisDirection(NeutralAxis.RX, 1) to LogicalControl.BUTTON_RB // extra
                )
            )
            val s = GamepadSnapshot.fromPhysicalInput(
                emptySet(),
                mapOf(
                    NeutralAxis.X to 1.0f,
                    NeutralAxis.Y to 0f,
                    NeutralAxis.RX to 0.8f
                ),
                mapping
            )
            assertThat(s.buttons[LogicalControl.DPAD_RIGHT.index]).isEqualTo(1.0f)
