...
/**
 * JVM unit tests for [SearchPresenter] state transitions, debounce,
 ...
 * made. The presenter runs on a virtual-time [StandardTestDispatcher], so
 * [TestCoroutineScheduler.advanceTimeBy]/[TestCoroutineScheduler.advanceUntilIdle]
 * drive the 300ms debounce and coalescing deterministically.
 */
@OptIn(ExperimentalCoroutinesApi::class)
@DisplayName("SearchPresenter — state, debounce, pagination, error, retry")
class SearchPresenterTest {
    ...
    /** Virtual-time scheduler driving the presenter's [StandardTestDispatcher]. */
    private lateinit var scheduler: TestCoroutineScheduler
    /** Test scope with a virtual-time dispatcher so debounce delays are driven by [scheduler]. */
    ...
    fun `initial state is idle with empty query`() {
        val presenter = SearchPresenter(scope = testScope, repository = MockRepository())
    ...
    fun `searching transitions through loading to loaded state`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = repo, hideUnsupportedSystems = { false })

        presenter.onQueryChanged("Zelda")
        presenter.submitQuery()

        // submitQuery launches on the virtual-time dispatcher; drain it.
    ...
    fun `retry re-issues the last search and recovers from error`() {
        ...
        // First search fails
        presenter.onQueryChanged("retry")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.error).isEqualTo(RommApiError.NETWORK_ERROR)
    ...
    fun `blank query resets results and stays idle`() {
        ...
        // Load results
        presenter.onQueryChanged("something")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.roms).hasSize(10)

        // Clear query
        presenter.onQueryChanged("")
        val state = presenter.uiState.value
        assertThat(state.query).isEmpty()
        assertThat(state.roms).isEmpty()
        assertThat(state.total).isEqualTo(0)
    ...
    fun `new query cancels previous in-flight search`() {
        ...
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.roms[0].title).isEqualTo("Game 100")

        // Second search (cancels first)
        presenter.onQueryChanged("second")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.roms[0].title).isEqualTo("Game 200")
    }
    ...
    fun `loadMore appends next page of results`() {
        ...
        // First page
        presenter.onQueryChanged("multi")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.roms).hasSize(20)
        assertThat(presenter.uiState.value.rawFetchedCount).isEqualTo(20)
        assertThat(presenter.uiState.value.total).isEqualTo(60)
    ...
    fun `loadMore is a no-op when all results are loaded`() {
        ...
        presenter.onQueryChanged("small")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.roms).hasSize(5)
        assertThat(presenter.uiState.value.total).isEqualTo(5)
    ...
    fun `empty results list with zero total shows empty state`() {
        ...
        presenter.onQueryChanged("nothing")
        presenter.submitQuery()
    ...
    fun `query is reflected immediately in uiState before debounce`() {
        ...
        presenter.onQueryChanged("partial")
        assertThat(presenter.uiState.value.query).isEqualTo("partial")
    ...
    fun `debounced auto-search fires after 300ms of inactivity`() {
        ...
        presenter.onQueryChanged("debounce")

        // Before 300ms: no search should have fired
        scheduler.advanceTimeBy(250)
        assertThat(presenter.uiState.value.roms).isEmpty()
    ...
    fun `rapid query changes only fire one search for the final value`() {
        ...
        scheduler.advanceTimeBy(100)
        presenter.onQueryChanged("ab")
        scheduler.advanceTimeBy(100)
        presenter.onQueryChanged("abc")
    ...
    fun `submitQuery trims and searches the current query`() {
        ...
        presenter.onQueryChanged("  trimmed  ")
        presenter.submitQuery()

        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.roms).hasSize(3)
    }
    ...
    fun `blank query cancels in-flight search so stale results do not overwrite idle`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = repo)
        presenter.onQueryChanged("hung")
        presenter.submitQuery()
        // The search coroutine starts on the virtual dispatcher and blocks on job.join().
        scheduler.advanceUntilIdle()
    ...
    fun `stale loadMore response is discarded when query changes mid-pagination`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = blockingRepo, hideUnsupportedSystems = { false })
        presenter.onQueryChanged("original")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.roms).hasSize(10)
    ...
    fun `stale loadMore response is discarded when query is cleared mid-pagination`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = blockingRepo, hideUnsupportedSystems = { false })
        presenter.onQueryChanged("loadtest")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.roms).hasSize(10)
    ...
    fun `raw trailing space survives debounce completion`() {
        ...
        presenter.onQueryChanged("sonic advance ")
        scheduler.advanceTimeBy(350) // Past debounce
    ...
    fun `raw input survives submit and retry`() {
        ...
        presenter.onQueryChanged("mega man ")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.query).isEqualTo("mega man ")
        assertThat(presenter.uiState.value.activeQuery).isEqualTo("mega man")
        assertThat(presenter.uiState.value.error).isEqualTo(RommApiError.NETWORK_ERROR)
    ...
    fun `pagination uses normalized active term`() {
        ...
        presenter.onQueryChanged("gta ")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.query).isEqualTo("gta ")
        assertThat(presenter.uiState.value.activeQuery).isEqualTo("gta")
    ...
    fun `toggle OFF preserves all search results including unsupported platforms`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = repo, hideUnsupportedSystems = { false })
        presenter.onQueryChanged("retro")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
    ...
    fun `toggle ON filters out unsupported platform results`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = repo, hideUnsupportedSystems = { true })
        presenter.onQueryChanged("retro")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
    ...
    fun `toggle ON filters unsupported from paginated loadMore`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = repo, hideUnsupportedSystems = { true })
        presenter.onQueryChanged("all")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()

        // After first page: 2 supported out of 4 raw
        assertThat(presenter.uiState.value.roms).hasSize(2)
        assertThat(presenter.uiState.value.rawFetchedCount).isEqualTo(4)
        assertThat(presenter.uiState.value.roms.map { it.title }).containsExactly("Pokemon", "Zelda LOR")
    ...
    fun `toggle ON with all unsupported results returns empty`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = repo, hideUnsupportedSystems = { true })
        presenter.onQueryChanged("console")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
    ...
    fun `toggle lambda is evaluated at search time not construction time`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = repo, hideUnsupportedSystems = { hideFlag })
        presenter.onQueryChanged("first")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.roms).hasSize(2)
        ...
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.roms).hasSize(1)
        assertThat(presenter.uiState.value.roms[0].title).isEqualTo("Pokemon")

        // Flip toggle OFF — next search should show all again
        hideFlag = false
        repo.enqueue(LibraryResult.Success(RomPage(mixedRoms, 2)))
        presenter.onQueryChanged("third")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
        assertThat(presenter.uiState.value.roms).hasSize(2)
    }
    ...
    fun `pagination uses raw offset not filtered display size, no duplicate refetch`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = repo, hideUnsupportedSystems = { true })
        presenter.onQueryChanged("mixed")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
    ...
    fun `all unsupported pages progress via raw offset and eventually exhaust`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = repo, hideUnsupportedSystems = { true })
        presenter.onQueryChanged("allunsupported")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()

        // After first page: 0 visible, rawFetchedCount=2
        assertThat(presenter.uiState.value.roms).isEmpty()
        assertThat(presenter.uiState.value.rawFetchedCount).isEqualTo(2)
    ...
    fun `toggle OFF preserves rawFetchedCount equal to display count`() {
        ...
        assertThat(presenter.uiState.value.roms).hasSize(3)
        assertThat(presenter.uiState.value.rawFetchedCount).isEqualTo(3)
    ...
    fun `label semantics toggle ON first page — explicit state true, visible count less than rawFetched`() {
        ...
        val presenter = SearchPresenter(scope = testScope, repository = repo, hideUnsupportedSystems = { true })
        presenter.onQueryChanged("pokemon")
        presenter.submitQuery()
        scheduler.advanceUntilIdle()
    ...
    fun `label semantics toggle ON with pagination — visible count updates as supported results load`() {
        ...
        assertThat(presenter.uiState.value.hideUnsupportedSystems).isTrue()
        assertThat(presenter.uiState.value.roms).hasSize(2)
        assertThat(presenter.uiState.value.rawFetchedCount).isEqualTo(4)
    ...
    fun `preference change between search and loadMore updates explicit state`() {
        ...
        assertThat(presenter.uiState.value.hideUnsupportedSystems).isFalse()
        assertThat(presenter.uiState.value.roms).hasSize(2)
