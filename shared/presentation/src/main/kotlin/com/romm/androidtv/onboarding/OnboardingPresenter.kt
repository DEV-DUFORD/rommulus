 ...
 * section 10). Kept as `fun interface`s so the presenter never touches OkHttp,
 * repositories, or platform framework objects, and so tests can substitute
 ...
 * in [scope]. Every network boundary is abstracted behind the fun interfaces
 * above, and every low-level failure is mapped to a user-facing String here —
 * nothing network-specific ever reaches [OnboardingUiState].
 *
 * Concurrency rules:
...
class OnboardingPresenter(
    ...
    fun onBack() {
        ...
        when (state.step) {
            ...
            OnboardingStep.WELCOME -> {
                // No-op: the host screen finishes itself (Phase 5).
            }
        }
    }
    ...
    /**
     * Teardown hook invoked by the host (e.g. a lifecycle wrapper's onCleared):
     * the password String lives only in state; clear it on teardown so it is
